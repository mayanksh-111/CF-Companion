...
export function buildHeatmap(submissions: CfSubmission[]): string {
  ...
  const start = new Date(
    end.getTime() - (totalDays - 1) * DAY_MS
  );

  // Align to Sunday so every column represents a complete week.
  const startAligned = new Date(
    start.getTime() - start.getUTCDay() * DAY_MS
  );
  ...
  for (let week = 0; week < weeks; week++) {
    ...
    const firstDay = new Date(
      startAligned.getTime() + week * 7 * DAY_MS
    );
  ...
  const weekdayHtml = weekdayLabels
    .map(
      (day) => `<span class="weekday-label">${day}</span>`
    )
    .join("");
