...
import { CfRatingChange, CfSubmission, CfUser, fetchUserInfo, fetchUserRatingHistory, } from "../cfApi";
...
export class DashboardPanel {
  ...
  private async refresh(opts?: { background?: boolean }): Promise<void> {
    ...
    try{
      const [user, submissions, ratingHistory]: [ CfUser, CfSubmission[], CfRatingChange[]] = await Promise.all([
      ...
      this.panel.webview.html = renderDashboard(user, submissions, ratingHistory);
      this.schedulePoll();
    } 
    catch(err: any){
      if(!background){
        this.panel.webview.html = renderError(handle, err?.message ?? String(err));
      }
  ...
  private schedulePoll(): void {
    ...
    this.pollTimer = setTimeout(() => {this.refresh({ background: true }); }, delay);
  }
