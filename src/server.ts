...
export class ProblemServer implements vscode.Disposable {
  ...
  constructor(
    private readonly submitQueue?: SubmitJobQueue
  ) {
  ...
  start(port: number): void {
    ...
    this.httpServer = http.createServer((req, res) => {
      this.output.appendLine(
        `Incoming HTTP ${req.method} ${req.url} from ${req.socket.remoteAddress}`
      );
      ...
      if (url.pathname === "/submit-result" && req.method === "POST") {
        ...
        req.on("end", () => {
          try {
            ...
            this.output.appendLine(
              `Failed to parse /submit-result body: ${err}`
            );
      ...
      req.on("end", () => {
        if (body.length === 0) {
          this.output.appendLine(
            "HTTP POST arrived with an empty body — ignoring."
          );
    ...
    this.wss.on("connection", (socket: WebSocket, req) => {
      ...
      socket.on("message", (raw) => {
        this.handlePayload(raw.toString("utf8"), "ws");
      });
    ...
    this.httpServer.on("error", (err: NodeJS.ErrnoException) => {
      if (err.code === "EADDRINUSE") {
        vscode.window.showErrorMessage(
          `CF Companion: port ${port} is already in use. Change "cfCompanion.port" in settings or free the port.`
        );
      } else {
        vscode.window.showErrorMessage(
          `CF Companion server error: ${err.message}`
        );
      }
    ...
    this.httpServer.listen(port, "0.0.0.0", () => {
      this.output.appendLine(
        `Listening for WebSocket + HTTP on port ${port}`
      );
    });
  }
  ...
  private handlePayload(
    ...
    this.output.appendLine(
      `[${via}] payload received: ${raw.length} bytes`
    );

    if (raw.length > 0) {
      this.output.appendLine(
        `[${via}] preview: ${raw.slice(0, 200)}${raw.length > 200 ? "…" : ""}`
      );
    }
    ...
    try {
      ...
      this.output.appendLine(
        `Failed to parse incoming payload: ${err}`
      );
      return;
    }
    ...
    switch (data.type) {
      case "problem":
        this.output.appendLine(
          `Received problem ${data.contest_id}${data.problem_code}: ${data.problem_name}`
        );
      ...
      case "problem_error":
        this.output.appendLine(
          `Problem extraction failed for ${data.url}: ${data.error}`
        );
      ...
      case "contest_metadata":
        this.output.appendLine(
          `Contest metadata: ${data.contest_id} — ${data.name} (${data.problem_count} problems)`
        );
      ...
      case "contest_complete":
        this.output.appendLine(
          `Contest complete: ${data.contest_id} — ${data.successful}/${data.problem_count} succeeded`
        );
      ...
      case "compiler_list":
        this.output.appendLine(
          `Compiler list received: ${data.compilers.length} options`
        );
      ...
      case "submissions_scrape":
        this.output.appendLine(
          `Submissions scrape received: ${data.rows.length} row(s) for handle ${data.handle}`
        );
      ...
      default:
        this.output.appendLine(
          `Unrecognized message type: '${(data as any).type}'`
        );
    }
  }
